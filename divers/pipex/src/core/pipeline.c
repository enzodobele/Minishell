...
static int	setup_input_output(t_pipex *pipex, int *in_fd, int *outfile_error)
{
	int	outfile_test;
...
static int	execute_pipeline(t_pipex *pipex, int in_fd)
{
	int	i;
...
int	pipex(t_pipex *pipex)
{
	...
	setup_input_output(pipex, &in_fd, &outfile_error);
	last_exit_status = execute_pipeline(pipex, in_fd);
